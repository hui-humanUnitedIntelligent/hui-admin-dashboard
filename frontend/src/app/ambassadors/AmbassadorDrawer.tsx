...
const LEVEL_CONFIG: Record<AmbLevel, { color: string; bg: string; icon: string; label: string }> = {
...
// API-Response von action=detail
interface ApiDetail {
...
function calcLevel(n: number): AmbLevel {
...
export default function AmbassadorDrawer({ ambId, onClose, onRefresh }: DrawerProps) {
  ...
  const levelKey    = calcLevel(refCount);
  ...
  const borderLeft4  = '4px solid ' + lc.color;
  const borderAccent = '1px solid ' + lc.color + '44';
      ...
      <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 520, background: 'var(--bg-primary)', borderLeft: '1px solid var(--border)', zIndex: 200, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {/* Profil-Header */}
            <div style={{ padding: 20, borderLeft: borderLeft4, background: lc.bg }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                {profile.avatar_url ? (
                  <img src={String(profile.avatar_url)} alt={displayName}
                    style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: borderAccent }} />
                ) : (
                  <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 700, color: '#0F1117', flexShrink: 0, border: borderAccent }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 20, background: lc.bg, color: lc.color, border: borderAccent }}>{lc.icon} {lc.label}</span>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg-secondary)', flexShrink: 0 }}>
              {(['overview', 'Übersicht'] as const, ['referrals', 'Referrals'] as const, ['logs', 'Bewerbungen'] as const, ...[]).length > 0 && null}
              ...
              <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <button disabled={acting} onClick={() => setShowLevelMenu(v => !v)} style={{ padding: '7px 14px', borderRadius: 8, border: borderAccent, background: lc.bg, color: lc.color, fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
                      ⬆️ Level ▾
                    </button>
                    {showLevelMenu && (
