...
// Employee: Werke — Realtime + Soft-Delete Button
'use client';
...
function timeAgo(iso: string|null|undefined) {
  ...
  if(d===0)return'Heute';if(d<7)return`${d}d`;if(d<30)return`${Math.floor(d/7)}w`;return`${Math.floor(d/30)}mo`;
}
...
export default function EmployeeWorksPage() {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      const res = await fetch('/api/employee/works/delete', {
        method: 'POST',
  ...
  const filtered = works.filter(w => {
    if (hidden.has(w.id) || w.status === 'deleted') return false;
      ...
      <div style={{display:'flex',gap:8,marginBottom:14,flexWrap:'wrap',alignItems:'center'}}>
        {(['published','draft','all'] as const).map(t=>(
          <button key={t} onClick={()=>setTab(t)} style={{
            ...
            color:tab===t?'#0f1117':'var(--text-muted)',fontWeight:600,fontSize:12,cursor:'pointer',
          }}>{t==='published'?'Veröffentlicht':t==='draft'?'Entwürfe':'Alle'}</button>
        ))}
                ...
                <tr key={w.id} style={{borderBottom:'1px solid var(--border)'}}
                  ...
                  <td style={{padding:'10px 12px'}}>
                    {w.thumbnail_url
                      ...
                      :<div style={{width:40,height:40,borderRadius:6,background:'var(--bg-tertiary)',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text-muted)',fontSize:16}}>🖼️</div>}
                  </td>
                  ...
                  <td style={{padding:'10px 12px'}}>
                    <button onClick={()=>handleDelete(w.id)} disabled={deleting===w.id} style={{
                      padding:'4px 10px',borderRadius:6,border:'1px solid var(--red-dim,#ff6b6b44)',
                      background:'transparent',color:'var(--red,#ff6b6b)',fontSize:11,cursor:'pointer',opacity:deleting===w.id?0.5:1,
                    }}>{deleting===w.id?'…':'Löschen'}</button>
                  </td>
                </tr>
              ))}
            </tbody>
