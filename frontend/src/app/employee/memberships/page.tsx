...
// READ-ONLY Employee-Ansicht: Mitgliedschaften — REALTIME via useMemberships()
'use client';
...
export default function EmployeeMembershipsPage() {
  ...
  const filtered = memberships.filter(m => {
    ...
    return matchTab && (!q || (m.membership_type||'').toLowerCase().includes(q) || (m.user_id||'').toLowerCase().includes(q));
  });
  ...
  return (
    <EmployeeLayout title="Mitgliedschaften">
      ...
      <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <input type="text" placeholder="Typ oder User-ID…" value={search}
      ...
      <div style={{ fontSize:11, color:'var(--text-muted)', marginBottom:10 }}>
        🟢 Realtime aktiv · {filtered.length} Mitgliedschaften
      </div>
