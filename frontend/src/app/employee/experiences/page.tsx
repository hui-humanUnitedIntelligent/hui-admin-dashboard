...
export default function EmployeeErlebnisseProjektePage() {
  ...
  const tabBanners: Partial<Record<TabKey, { bg: string; border: string; color: string; text: string }>> = {
    ...
    deleted:   { bg: 'rgba(255,107,107,0.06)', border: 'var(--red)',  color: 'var(--red)',  text: '🗑 Hier siehst du gelöschte Einträge.' },
    flagged:   { bg: 'rgba(247,183,49,0.08)',  border: 'var(--gold)', color: 'var(--gold)', text: '⚑ Gemeldete Einträge sind versteckt. Du kannst die Meldung auflösen.' },
    ...
    >
      ...
      <div style={{ background: 'var(--bg-secondary)', borderRadius: 10, border: '1px solid var(--border)', overflow: 'hidden' }}>
        ...
        <div style={{ textAlign: 'center', padding: '60px 24px' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>Noch keine Daten</div>
          ...
          <div style={{ marginTop: 16, display: 'inline-flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
            {['experiences', 'projects', 'initiatives'].map(t => (
              <span key={t} style={{ padding: '4px 12px', borderRadius: 99, background: 'rgba(78,205,196,0.08)', border: '1px solid rgba(78,205,196,0.20)', color: 'var(--accent)', fontSize: 11, fontWeight: 600 }}>{t}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* ── Info ── */}
      <div style={{ marginTop: 14, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.20)', borderRadius: 10, padding: '11px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 16 }}>ℹ️</span>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.5 }}>
          <strong style={{ color: '#FBBF24' }}>Vorbereitet für:</strong>
          {' '}Tabellen <code>experiences</code>, <code>projects</code> und <code>initiatives</code>.
