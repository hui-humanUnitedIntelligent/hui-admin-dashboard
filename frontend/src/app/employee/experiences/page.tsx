...
export default function EmployeeErlebnisseProjektePage() {
  ...
  const handleApprove = async (entry: HuiEntry) => {
    ...
    if (ok) { showToast(`✅ Freigegeben: ${entry.title || 'Eintrag'}`, 'success'); refetchAll(); }
  ...
  const handleDelete = async (entry: HuiEntry) => {
    ...
    const ok = await entryAction(action, entry.id);
    if (ok) { showToast('🗑 Gelöscht', 'info'); setTimeout(() => setLocalDeleted(new Set()), 3000); }
    else     { showToast('Fehler beim Löschen', 'error'); setLocalDeleted(prev => { const s2 = new Set(prev); s2.delete(entry.id); return s2; }); }
  };
