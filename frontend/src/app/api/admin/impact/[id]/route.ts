// frontend/src/app/api/admin/impact/[id]/route.ts
// DELETE /api/admin/impact/[id] — Hard-Delete (Superadmin only)
...
export async function DELETE(
  ...
  try {
    ...
    const { data: row } = await sb.from('impact_applications').select('*').eq('id', id).single();
