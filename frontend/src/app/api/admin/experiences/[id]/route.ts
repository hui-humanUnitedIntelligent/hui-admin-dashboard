// frontend/src/app/api/admin/experiences/[id]/route.ts
// DELETE /api/admin/experiences/[id] — Hard-Delete (Superadmin only)
...
export async function DELETE(
  ...
  try {
    ...
    const { data: row } = await sb.from('experiences').select('*').eq('id', id).single();
